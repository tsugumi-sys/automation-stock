...
def lstm():
    ...
    failed_symbol = []
    symbols = pd.read_csv('../2021_rakuten_stock_list.csv')
    for symbol in symbols['symbol']:
        print('-'*50, symbol, '-'*50)
        try:
            df = yf.download(symbol, start=dt.datetime.now() - dt.timedelta(days=120), end=dt.datetime.now(), interval='1d')
            preds = make_prediction(df, model)
            pred = preds[-1] if len(preds) > 0 else None
            if pred:
                print(round(pred, 5))
                pred_df.loc[symbol, 'pred_val'] = round(pred, 5)
        except:
            print('Failed ', symbol)
            failed_symbol.append(symbol)
    pred_df.to_csv('usstock-pred.csv')
    failed_df = pd.DataFrame({'symbol': failed_symbol})
    failed_df.to_csv('usstock-fail.csv')
    
def nikkei_lstm():
    model = load_model('../models/model14/model.h5')
    pred_df = pd.DataFrame()
    failed_symbol = []
    symbols = pd.read_csv('../../../../symbols/nikkei255.csv', index_col=0)
    for symbol in symbols['symbol']:
        print('-'*50, symbol, '-'*50)
        try:
            df = yf.download(symbol, start=dt.datetime.now() - dt.timedelta(days=120), end=dt.datetime.now(), interval='1d')
            ...
            print('Failed ', symbol)
            failed_symbol.append(symbol)
    pred_df.to_csv('nikkei-pred.csv')
    failed_df = pd.DataFrame({'symbol': failed_symbol})
    failed_df.to_csv('nikkei-fail.csv')
